 ...
 * \ingroup abr-test
 *
 * \brief ABR regression test suite
 */
class AbrRegressionTestSuite : public TestSuite
{
  public:
    AbrRegressionTestSuite()
        : TestSuite("routing-abr-regression", SYSTEM)
    {
        SetDataDir(NS_TEST_SOURCEDIR);
        // General RREQ-RREP-RRER test case
        AddTestCase(new ChainRegressionTest("abr-chain-regression-test"), TestCase::QUICK);
...
} g_abrRegressionTestSuite; ///< the test suite
...
ChainRegressionTest::ChainRegressionTest(const char* const prefix,
    ...
    : TestCase("ABR chain regression test"),
      m_nodes(nullptr),
      m_prefix(prefix),
      m_time(t),
      m_size(size),
...
ChainRegressionTest::CreateDevices()
{
    ...
    // 2. Setup TCP/IP & ABR
    AbrHelper abr; // Use default parameters here
    InternetStackHelper internetStack;
    internetStack.SetRoutingHelper(abr);
    ...
    streamsUsed += abr.AssignStreams(*m_nodes, streamsUsed);
    // ABR uses m_size more streams

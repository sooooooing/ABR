 ...
 */
#include "ns3/abr-neighbor.h"
#include "ns3/abr-packet.h"
#include "ns3/abr-rqueue.h"
#include "ns3/abr-rtable.h"
...
{
    TypeHeaderTest()
        : TestCase("ABR TypeHeader")
    {
    }

    void DoRun() override
    {
        TypeHeader h(ABRTYPE_RREQ);
        NS_TEST_EXPECT_MSG_EQ(h.IsValid(), true, "Default header is valid");
        NS_TEST_EXPECT_MSG_EQ(h.Get(), ABRTYPE_RREQ, "Default header is RREQ");

        Ptr<Packet> p = Create<Packet>();
        p->AddHeader(h);
        TypeHeader h2(ABRTYPE_RREP);
 ...
 * \ingroup abr-test
 *
 * \brief Unit test for ABR routing table entry
 */
...
struct AbrRqueueTest : public TestCase
{
    AbrRqueueTest()
        : TestCase("Rqueue"),
          q(64, Seconds(30))
...
AbrRqueueTest::DoRun()
{
    ...
    h.SetDestination(Ipv4Address("1.2.3.4"));
    h.SetSource(Ipv4Address("4.3.2.1"));
    Ipv4RoutingProtocol::UnicastForwardCallback ucb = MakeCallback(&AbrRqueueTest::Unicast, this);
    Ipv4RoutingProtocol::ErrorCallback ecb = MakeCallback(&AbrRqueueTest::Error, this);
    ...
    Simulator::Schedule(q.GetQueueTimeout() + Seconds(1), &AbrRqueueTest::CheckTimeout, this);
 ...
 * \brief Unit test for ABR routing table entry
 */
struct AbrRtableEntryTest : public TestCase
{
    AbrRtableEntryTest()
 ...
 * \ingroup abr-test
 *
 * \brief Unit test for ABR routing table
 */
struct AbrRtableTest : public TestCase
{
    AbrRtableTest()
        : TestCase("Rtable")
    {
 ...
 * \ingroup abr-test
 *
 * \brief ABR test suite
 */
class AbrTestSuite : public TestSuite
{
  public:
    AbrTestSuite()
        : TestSuite("routing-abr", UNIT)
    {
        ...
        AddTestCase(new AbrRqueueTest, TestCase::QUICK);
        AddTestCase(new AbrRtableEntryTest, TestCase::QUICK);
        AddTestCase(new AbrRtableTest, TestCase::QUICK);
    }
} g_abrTestSuite; ///< the test suite

...
Ad Hoc On-Demand Distance Vector (ABR)
---------------------------------------

This model implements the base specification of the Ad Hoc On-Demand
Distance Vector (ABR) protocol. The implementation is based on
:rfc:`3561`.

The model was written by Elena Buchatskaia and Pavel Boyko of ITTP RAS,
and is based on the ns-2 ABR model developed by the CMU/MONARCH group
and optimized and tuned by Samir Das and Mahesh Marina, University of
Cincinnati, and also on the ABR-UU implementation by Erik Nordström of
Uppsala University.

Model Description
*****************

The source code for the ABR model lives in the directory `src/abr`.

Design
++++++

Class ``ns3::abr::RoutingProtocol`` implements all functionality of
...
forwarding.  The first one, ``ns3::abr::RouteOutput``, is used for
locally originated packets, and the second one, ``ns3::abr::RouteInput``,
...
this functionality are attributes of ``ns3::abr::RoutingProtocol``.
...
ABR discovers routes on demand.  Therefore, the ABR model buffers all
packets while a route request packet (RREQ) is disseminated.
A packet queue is implemented in abr-rqueue.cc. A smart pointer to
...
* This ABR implementation can detect the presence of unidirectional
  ...
  This heuristic is taken from ABR-UU implementation and can be disabled.
...
the assertion from the ABR RFC that ABR works over UDP.  This model uses

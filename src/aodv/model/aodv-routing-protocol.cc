...
RoutingProtocol::SendRequest(Ipv4Address dst)
{
    ...
    // Send RREQ as subnet directed broadcast from each interface used by aodv / AODV가 바인딩된
    // 모든 인터페이스에서 전송
         ...
         ++j) // 모든 인터페이스에서 전송 (멀티 인터페이스 노드일 수 있으므로)
    {
        ...
        // IP헤더 -> AODV 타입 헤더 -> RREQ 헤더 -> 페이로드 순으로 구성
...
RoutingProtocol::RecvAodv(Ptr<Socket> socket)
{
    ...
    Ipv4Address sender = inetSourceAddr.GetIpv4(); // sender는 AODV 패킷을 직접 보낸 이전 hop
                                             ...
                                             bool gratRep)
{
    ...
    socket->SendTo(packet,
                   ...
                   InetSocketAddress(toOrigin.GetNextHop(),
                                     AODV_PORT)); // origin으로 가기 위한 next hop에 RREP 전송
...
RoutingProtocol::SendPacketFromQueue(Ipv4Address dst, Ptr<Ipv4Route> route)
{
    ...
    while (m_queue.Dequeue(dst, queueEntry)) // m_queue = AODV의 송신 대기 큐 (RREQ 중에는 데이터
                                             // 패킷을 바로 못보내고 dst 기준으로 큐에 저장)
    {
        ...
        // 원래 Ipv4가 패킷을 보낼때 호출하려는 함수(AODV가 가로채서 잠깐 보류해둔것)
